 ...
 * @param {Object} pdfUrl - { view: string, download: string }
 */
async function sendTaskEmail(employee, task, pdfUrl) {
  try {
    ...
    const hasPdf = pdfUrl && pdfUrl.view;
        ...
        <div style="font-family: 'Segoe UI', Arial, sans-serif; background-color: #f9f9f9; padding: 20px; border-radius: 8px;">
          <div style="max-width: 600px; margin: auto; background: #fff; border-radius: 10px; box-shadow: 0 0 8px rgba(0,0,0,0.1); padding: 25px;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin: 15px 0;">
              <tr><td style="padding: 8px; font-weight: bold; border: 1px solid #eee;">Task:</td>
                  <td style="padding: 8px; border: 1px solid #eee;">${task.title}</td></tr>

              <tr><td style="padding: 8px; font-weight: bold; border: 1px solid #eee;">Deadline:</td>
                  <td style="padding: 8px; border: 1px solid #eee;">${task.dueDate ? new Date(task.dueDate).toLocaleDateString() : "N/A"}</td></tr>

              <tr><td style="padding: 8px; font-weight: bold; border: 1px solid #eee;">Priority:</td>
                  <td style="padding: 8px; border: 1px solid #eee;">${task.priority || "N/A"}</td></tr>

              <tr><td style="padding: 8px; font-weight: bold; border: 1px solid #eee;">Estimated Hours:</td>
                  <td style="padding: 8px; border: 1px solid #eee;">${task.estimatedHours || "N/A"}</td></tr>
            </table>
              ...
              hasPdf
                ? `
                  <p style="margin-top: 15px;">
                    📄 <strong>Task Report Available</strong><br/><br/>
                    <a href="${pdfUrl.view}" target="_blank" style="color: #007BFF; text-decoration: none;">🔍 View PDF</a><br/>
                    <a href="${pdfUrl.download}" target="_blank" style="color: #28a745; text-decoration: none;">⬇ Download PDF</a>
                  </p>
                `
